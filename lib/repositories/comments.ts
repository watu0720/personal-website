...
/** Check body contains only allowed links (http/https). */
export function checkBodyLinks(body: string): { ok: boolean; hasLinks: boolean } {
  const urlRe = /https?:\/\/[^\s]+/gi;
  const matches = body.match(urlRe);
  if (!matches) return { ok: true, hasLinks: false };
  const allOk = matches.every((u) => /^https?:\/\//i.test(u));
  return { ok: allOk, hasLinks: true };
}
